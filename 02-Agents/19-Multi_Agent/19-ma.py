...
def create_supervisor_workflow(model):
...
def create_sequential_workflow(model):
...
# %%
def create_tool_based_workflow(model):
    @tool
    ...
    def general_agent(query: str, state: Annotated[dict, InjectedState]) -> str:
        ...
        general_response = model.invoke(
            [{'role': 'system', 'content': 'You are a helpful general assistant.'}, {'role': 'user', 'content': query}]
        )
...
# Choose a model to use consistently, e.g., ChatOpenAI or ChatOllama
# model = ChatOpenAI(model="gpt-4")
model = ChatOllama(model='llama3.2')

# Example 1: Supervisor Architecture
print('=== Supervisor Architecture Example ===')
supervisor_workflow = create_supervisor_workflow(model)
...
sequential_workflow = create_sequential_workflow(model)

result = sequential_workflow.invoke({'messages': [{'role': 'user', 'content': 'Organize a team building event'}]})
print('Sequential Result:', result['messages'][-1].content)
# %%
# Example 3: Tool-Based Workflow
print('\n=== Tool-Based Workflow Example ===')
tool_workflow = create_tool_based_workflow(model)
