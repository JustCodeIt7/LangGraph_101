...
def demonstrate_forced_tool_use():
    ...
    # Create an agent without forcing tool choice to avoid infinite loops
    # Instead, we'll structure the prompt to encourage tool use
    agent = create_react_agent(model=model, tools=tools)

    # Run an example with a message that naturally encourages greeting
    user_message = "Hi, my name is Alice. Please greet me properly."
    console.print(f"[bold]User:[/bold] {user_message}\n")
    console.print("[bold]Agent (encouraged to use greet tool):[/bold]")

    # Add recursion limit as a safety measure
