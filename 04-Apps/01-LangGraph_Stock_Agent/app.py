...
def fetch_financial_statements(ticker: str, period: str = "yearly") -> dict:
    ...
    try:
        ...
        balance_items = ['Total Assets', 'Total Liabilities Net Minority Interest',
                        'Stockholders Equity', 'Cash And Cash Equivalents']
        income_items = ['Total Revenue', 'Net Income', 'Operating Income',
                       'Gross Profit', 'EBITDA']
        cashflow_items = ['Operating Cash Flow', 'Investing Cash Flow',
...
def analyze_financials_node(state: AgentState) -> AgentState:
    ...
    # Initialize Ollama LLM (using llama2 by default)
...
def main():
    ...
    ticker = st.sidebar.text_input("Stock Ticker", value="AAPL", help="Enter ticker symbol (e.g., AAPL, MSFT, TSLA)").upper()
    ...
    # Info box
    st.sidebar.info("""
    **Note:** Make sure Ollama is running locally with llama2 model installed.
    ...
    if analyze_button:
        with st.spinner("🤖 AI Agent is analyzing..."):
            ...
            try:
                ...
                st.info("Make sure Ollama is running and llama2 model is installed.")

    # Footer
    st.markdown("---")
    st.markdown("Built with ❤️ using Streamlit, LangChain, LangGraph & Ollama")
