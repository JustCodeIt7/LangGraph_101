...
Usage:
    python voice_agent_local.py <path_to_audio_file> [--model llama3.2]

Requirements (all local):
    pip install mlx-whisper langchain-ollama langgraph rich python-dotenv
...
def get_weather(location: str) -> str:
    ...
    # In production, call a real local or remote weather API.
...
def text_to_speech(text: str, output_path: str = "response.aiff") -> None:
    ...
    # `say` writes audio to a file; -v chooses voice, -o specifies output path.
    subprocess.run(
...
def run_voice_agent(
    ...
    # Step 1 — Speech to text (local Whisper via MLX)
    ...
    # Ensure clean string for TTS output
...
def main() -> None:
    ...
    run_voice_agent(args.audio, llm_model=args.model, output_path=args.out)
